...
export function extractCifraClubKey(content: string): string | null {
  ...
  // 1. Explicit metadata: "Tom: C", "Tom:  G#m", "Tom : Am", "tom G sol"
  const tomMatch = content.match(/^\s*Tom\s*:\s*([A-G][#b]?m?)/im) || content.match(/^\s*tom\s+([A-G][#b]?m?)/im);
  if (tomMatch) {
    const key = tomMatch[1];
    if (COMMON_KEYS.includes(key)) return key;
  }
  ...
  // 3. Fallback: First recognized chord root in lines that are not headers
  const lines = content.split(/\r?\n/);
  for (const line of lines) {
    if (/^\s*(?:T[ií]tulo|Artista|Autor|Tom|tom|BPM|Tempo|Capo)/i.test(line)) continue;
    const chordMatches = line.match(/\b([A-G][#b]?m?)(?:[0-9]|maj|min|dim|aug|sus|\/|\b)/g);
    if (chordMatches) {
      for (const cm of chordMatches) {
        const rootMatch = cm.match(/^([A-G][#b]?m?)/);
        if (rootMatch && COMMON_KEYS.includes(rootMatch[1])) {
          return rootMatch[1];
        }
      }
    }
...
export function preprocessChordSheet(raw: string): string {
  ...
  // Strip standalone metadata headers
  text = text.replace(/^\s*T[ií]tulo\s*:\s*.*$/gim, "");
  text = text.replace(/^\s*Artista\s*:\s*.*$/gim, "");
  text = text.replace(/^\s*Autor\s*:\s*.*$/gim, "");
  text = text.replace(/^\s*Tom\s*:\s*[A-G][#b]?m?\s*$/gim, "");
  text = text.replace(/^\s*tom\s+[A-G][#b]?m?.*$/gim, "");
